...
// S6a: 터치 기기에서는 키 이름(K/E/J) 대신 버튼 이름(반사/추적/공격)으로 말한다.
...
const PHASE_ACTION_TEXT = Object.freeze({
  ...
  touch: Object.freeze({
    'reflect-scan': '빔을 반사 버튼으로',
    'trace-roster': '진짜 기록에 추적 버튼',
    'approval-core': '코어에 공격 버튼'
  })
});
...
const PHASE_VERB_LABEL = Object.freeze({
  ...
  touch: Object.freeze({
    'reflect-scan': '빔이 온다 — 반사 버튼!',
    'trace-roster': '진짜 기록에 추적 버튼!',
    'approval-core': '코어 개방 — 공격 버튼!'
  })
});
...
const PHASE_ENTRY_LINES = Object.freeze({
  ...
  touch: Object.freeze({
    'reflect-scan': {
      id: 'boss-entry-reflect',
      speaker: 'DOT',
      textKo: '감독관은 베기가 안 통해. 붉은 스캔 빔이 켜지면 — 반사 버튼을 눌러.',
      durationMs: 4600
    },
    'trace-roster': {
      id: 'boss-entry-trace',
      speaker: 'DOT',
      textKo: '복제 명단 셋. 진짜 승인 기록만 추적 버튼으로 짚어.',
      durationMs: 3800
    },
    'approval-core': {
      id: 'boss-entry-core',
      speaker: 'DOT',
      textKo: '코어가 열렸다 — 지금이야, 공격 버튼!',
      durationMs: 3600
    }
  })
});
...
const PHASE_MISMATCH_LINES = Object.freeze({
  ...
  touch: Object.freeze({
    'reflect-scan': {
      id: 'boss-retry-reflect',
      speaker: 'DOT',
      textKo: '베기 말고! 빔이 켜지는 순간 반사 버튼이야.',
      durationMs: 2800
    },
    'trace-roster': {
      id: 'boss-retry-trace',
      speaker: 'DOT',
      textKo: '지금은 칼 넣어. 진짜 기록만 추적 버튼으로 짚어.',
      durationMs: 2800
    },
    'approval-core': {
      id: 'boss-retry-core',
      speaker: 'DOT',
      textKo: '지금은 공격 버튼! 열린 코어를 베어.',
      durationMs: 2800
    }
  })
});
...
export function bossObjectiveText(bossState, device = 'desktop') {
  const phase = activePhase(bossState);
  const actionText = phase ? PHASE_ACTION_TEXT[deviceKey(device)][phase.id] : null;
...
export function bossVerbFor(bossState) {
  const phase = activePhase(bossState);
  if (!phase || !PHASE_ACTION_TEXT.desktop[phase.id]) return null;
...
export function bossGuidancePrompts(bossState, device = 'desktop') {
  const phase = activePhase(bossState);
  if (!phase) return [];
  const prompts = [];
  const verb = bossVerbFor(bossState);
  if (verb) prompts.push({ id: `boss-verb-${phase.id}`, label: PHASE_VERB_LABEL[deviceKey(device)][phase.id] });
  if ((bossState.knowledge?.length ?? 0) === 0) prompts.push(ORDER_HINT_PROMPT[deviceKey(device)]);
  return prompts;
}

// 보스 이벤트·상태 → 무전 줄 목록. 진입·단계 전환·격려는 장면당 1회, 오입력은 3초 간격.
// 같은 줄 id는 기기 표현이 바뀌어도 다시 반복하지 않는다(회전·뷰포트 전환에도 안내는 1회).
export function createBossGuidance() {
  ...
  return Object.freeze({
    ...
    observe({ events = [], state = null, device = 'desktop' } = {}) {
      const lines = [];
      ...
      const entryLines = PHASE_ENTRY_LINES[deviceKey(device)];
      if (phase && entryLines[phase.id]) {
        const entry = once(entryLines[phase.id]);
        if (entry) lines.push(entry);
      }
      return lines;
    },
    // 오입력 — 현재 단계 동사와 다른 전투 입력이 오면 재안내 한 줄(3초 이내 반복 금지).
    noteMismatch(actionType, state, device = 'desktop') {
      const phase = activePhase(state);
      const mismatchLines = PHASE_MISMATCH_LINES[deviceKey(device)];
      if (!phase || !mismatchLines[phase.id]) return null;
