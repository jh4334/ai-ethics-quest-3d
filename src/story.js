...
function withAndParticle(nameKo) {
  if (!nameKo) {
    return '';
  }
  const last = nameKo.charCodeAt(nameKo.length - 1);
  const hasBatchim = last >= 0xac00 && last <= 0xd7a3 && (last - 0xac00) % 28 !== 0;
  return `${nameKo}${hasBatchim ? '과' : '와'}`;
}
...
export function getGateDialog(progress, topicId) {
  ...
  if (status === 'need-intro') {
    return { kind: 'need-intro', titleKo: quest.gateLabelKo, linesKo: [`먼저 ${withAndParticle(quest.npcNameKo)} 이야기해 보세요.`] };
  }
  if (status === 'need-tool') {
...
export function getStoryObjective(progress) {
  ...
  for (const topicId of STORY_TOPIC_ORDER) {
    ...
    if (status === 'need-intro') {
      return `「${quest.questTitleKo}」 — ${withAndParticle(quest.npcNameKo)} 이야기하세요.`;
    }
    if (status === 'need-tool') {
