...
- 1장 첫 화면은 세 겹 이상의 부유 캠퍼스 깊이, 황혼 하늘, GLB 외벽·수목·가구, 벽돌·콘크리트·유리·금속·목재 구분, 따뜻한 기억 동선, 3인칭 어깨너머 구도로 재구성했다. 최신 `cb27347172b1` 패스는 건축 재질 역할명이 누락돼 ambientCG PBR 교체가 실행되지 않던 결함을 고쳤고, 첫 상판을 비정형 벽돌 PBR 면·18개 석재 포장·16개 높이차 암석 림으로 바꿨다. 레퍼런스 이미지를 2D 배경으로 사용하지 않았다.
- 최신 P0 5/5 캡처는 Intel Iris Xe D3D11 하드웨어 WebGL에서 생성했으며 콘솔 오류와 4xx/5xx 실패 응답은 각각 0건이다.
...
- Playwright E2E: 리부트 3/3, 슬라이스 2/2, 캠페인 7/7, 폴리시 11/11, 합계 23/23 통과. 결과 패널의 같은 텍스트를 매 프레임 다시 쓰던 동작을 제거해 CI 스크린샷 안정성 대기를 고정했고, 수정 후 CI 모드 리부트 3/3이 통과했다. 폴리시 통합 실행 중 OneDrive가 기존 증빙 PNG를 두 차례 잠근 파일 I/O 오류가 있었지만, 해당 시나리오는 분리 재실행에서 각각 통과했다.
- GitHub Actions는 23개를 한 무제한 단계에서 돌리지 않고 캠페인·시각/접근성·저장/오프라인·결말 파일로 나눴다. 각 단계는 30분, 전체 빌드 작업은 120분으로 제한하며 첫 확정 실패에서 중단한다.
- `npm run build`: 통과, 172개 모듈. 리부트 청크 328,860바이트(gzip 109,097바이트), Three.js 681,282바이트(gzip 172,207바이트).
- `npm run smoke`, `npm run slice:gate`: 통과. H-17 기본 진입점과 legacy 롤백 경로를 확인했다.
- 현재 dist 프로덕션 검증: v5 저장 키, v12 서비스워커 캐시, 오프라인 재접속 뒤 `chapter-3:start` 복원 통과. lazy 환경 에셋의 응답 캐시 쓰기를 기다리고, 첫 로드가 서비스워커 제어보다 먼저 끝나면 로더가 URL을 보류했다가 `controllerchange` 뒤 전달한다. 워커는 동일 출처 환경 경로만 검증해 저장한다. 이 늦은 제어권 회귀를 두 계약 테스트로 고정했으며 CI 모드 오프라인 Playwright는 3/3 통과했다.
...
- 초기 셸: 1,366,114바이트, 12파일(1.30MiB).
- 설치 프리캐시: 37,811,921바이트, 56파일(36.06MiB).
...
- 전체 분류 자산: 51,066,632바이트, 98파일(48.70MiB).
...
- 최신 P0 5개 화면 최대치: 197 draw calls, 152,654 triangles, 4 lights, Intel Iris Xe D3D11 직접 rAF p95 16.7ms. 첫 조우 화면은 168 calls·138,631 triangles이며 모바일 첫 화면은 188 calls·149,674 triangles다.

## 남은 위험과 솔직한 품질 평가

- 최신 1장은 실제 PBR 바인딩, 비정형 석재 경로, 암석성 섬 가장자리로 평평한 청색 블록아웃 인상을 줄였다. 그러나 반복되는 로우폴리 외벽 모듈, 부족한 수직 지형·식생 밀도, 단순한 원거리 실루엣 때문에 콘셉트의 회화적 밀도와 영화적 조명에는 아직 못 미친다. 현재 평가는 완주 가능한 스타일라이즈드 로우폴리 3D 게임이며, 레퍼런스 충실도나 AAA 수준이라고 주장하지 않는다.
