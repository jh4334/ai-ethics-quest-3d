...
- 키보드: 최적화 바이너리에서 1~6장 각각 실제 이동·전투·입력·저장 후 재접속·다음 장 전환 통과. 합계 자동 검증 시간 390.2초.
- 터치: 같은 범위를 390×844에서 통과. 합계 자동 검증 시간 258.0초, 터치 스틱 연결, 모든 주요 버튼 최소 44px.
- 장별 자동 검증 시간(키보드/터치): 1장 99.5/61.4초, 2장 56.2/37.8초, 3장 56.2/35.3초, 4장 66.3/43.1초, 5장 70.1/53.2초, 6장 41.9/27.2초.
...
- 설치 프리캐시: 37,806,365바이트, 56파일(36.05MiB).
...
- 전체 분류 자산: 50,842,240바이트, 97파일(48.49MiB).
- 최신 캡처의 장별 데스크톱 전송량: 1장 47.5MiB, 2장 32.0MiB, 3장 34.9MiB, 4장 40.5MiB, 5장 36.8MiB, 6장 41.3MiB. 로컬 서버의 장별 독립 진입 측정이므로 합산 다운로드로 해석하지 않는다.
- 12개 화면 최대치: 187 draw calls, 148,205 triangles, 4 lights, 데스크톱·모바일 직접 rAF p95 16.7ms.
...
- 48.49MiB 전체 자산과 36.05MiB 설치 프리캐시는 크게 줄었지만 학교망 첫 설치에는 여전히 부담이다. 장 환경 지연 로딩과 저화질 프로필은 작동하며, 남은 캐릭터 본체·의상 GLB의 Meshopt/Draco 검토와 KTX2 텍스처화, 실제 저사양 기기·학교망 검증이 남았다.
