...
- 1장 첫 화면은 세 겹 이상의 부유 캠퍼스 깊이, 황혼 하늘, GLB 외벽·수목·가구, 벽돌·콘크리트·유리·금속·목재 구분, 따뜻한 기억 동선, 3인칭 어깨너머 구도로 재구성했다. 레퍼런스 이미지를 2D 배경으로 사용하지 않았다.
...
- 키보드: 1~6장 각각 실제 이동·전투·입력·저장 후 재접속·다음 장 전환 통과. 합계 자동 검증 시간 542.5초.
- 터치: 같은 범위를 390×844에서 통과. 합계 자동 검증 시간 326.8초, 터치 스틱 연결, 모든 주요 버튼 최소 44px.
- 장별 자동 검증 시간(키보드/터치): 1장 166.7/99.0초, 2장 79.0/55.4초, 3장 77.5/77.8초, 4장 138.8/33.4초, 5장 51.2/39.3초, 6장 29.3/21.9초.
...
- `npm test`: 500/500 통과.
- `npm run test:h17`: 300/300 통과.
- `npm run test:legacy`: 200/200 통과.
- Playwright E2E: 리부트 3/3, 슬라이스 2/2, 캠페인 7/7, 폴리시 11/11, 합계 23/23 통과.
- `npm run build`: 통과, 172개 모듈. 리부트 청크 약 323.38kB(gzip 107.76kB), Three.js 681.28kB(gzip 173.47kB).
- `npm run smoke`, `npm run slice:gate`: 통과. H-17 기본 진입점과 legacy 롤백 경로를 확인했다.
- 현재 dist 프로덕션 검증: v5 저장 키, v12 서비스워커 캐시, 오프라인 재접속 뒤 `chapter-3:start` 복원 통과.
...
- 초기 셸: 1,360,558바이트, 12파일(1.30MiB).
- 설치 프리캐시: 51,171,365바이트, 56파일(48.80MiB).
- 장 환경: 13,035,875바이트, 41파일(12.43MiB).
- 캐릭터: 34,100,927바이트, 42파일(32.52MiB). 애니메이션: 15,709,880바이트, 2파일(14.98MiB).
- 전체 분류 자산: 64,207,240바이트, 97파일(61.23MiB).
- 최신 캡처의 장별 데스크톱 전송량: 1장 60.2MiB, 2장 44.8MiB, 3장 47.6MiB, 4장 53.3MiB, 5장 49.5MiB, 6장 47.3MiB. 로컬 서버의 장별 독립 진입 측정이므로 합산 다운로드로 해석하지 않는다.
- 12개 화면 최대치: 187 draw calls, 148,205 triangles, 4 lights, 직접 rAF p95 24.9ms. 1장 데스크톱은 187 calls, 148,205 triangles, 4 lights, p95 16.8ms였다.

## 남은 위험과 솔직한 품질 평가

- 최신 1장은 이전 화면보다 공간 깊이·진입 동선·황혼 조명·실제 GLB 밀도·재질 구분이 뚜렷하게 개선됐다. 그러나 콘셉트의 회화적 밀도와 고급 재질·식생·영화적 조명에는 아직 못 미친다. 현재 평가는 완주 가능한 스타일라이즈드 로우폴리 3D 게임이며, 레퍼런스 충실도나 AAA 수준이라고 주장하지 않는다.
- 61.23MiB 전체 자산과 48.80MiB 설치 프리캐시는 학교망 첫 설치에 부담이다. 장 환경 지연 로딩과 저화질 프로필은 작동하지만 캐릭터·애니메이션 Meshopt/Draco 또는 KTX2 압축, 실제 저사양 기기·학교망 검증이 남았다.
