...
- `npm run build`: 통과, 172개 모듈. 리부트 청크 328,256바이트(gzip 108,051바이트), Three.js 681,282바이트(gzip 171,556바이트).
- `npm run smoke`, `npm run slice:gate`: 통과. H-17 기본 진입점과 legacy 롤백 경로를 확인했다.
- 현재 dist 프로덕션 검증: v5 저장 키, v12 서비스워커 캐시, 오프라인 재접속 뒤 `chapter-3:start` 복원 통과. lazy 환경 에셋은 응답 수명주기 안에서 캐시 쓰기를 완료하도록 수정했으며, 해당 오프라인 Playwright 파일은 수정 후 3/3 통과했다. 첫 원격 CI에서 드러난 캐시 쓰기 취소 회귀를 재현 테스트로 고정했다.
...
- 초기 셸: 1,363,354바이트, 12파일(1.30MiB).
- 설치 프리캐시: 37,809,161바이트, 56파일(36.06MiB).
...
- 전체 분류 자산: 51,063,872바이트, 98파일(48.70MiB).
