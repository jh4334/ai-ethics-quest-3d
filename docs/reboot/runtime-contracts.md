...
- 저장 키: `h17.null.save.v5`; `schemaVersion`은 숫자 `5`다.
- 기존 v4 원문 백업 키: `h17.null.save.v4.backup`; 최초 마이그레이션 때 바이트 그대로 한 번만 복사하고 이후 쓰지 않는다.
- v4의 기존 5장 방송 체크포인트는 v5의 6장으로 옮긴다. 기존 캠페인 완료자는 새 5장에 `legacy-grandfathered` 결과 기록을 받아 장 순서와 엔딩을 보존한다.
...
- 관문 시도 기록 `gateAttempts`는 윤리적 선택과 분리한다. 쓰기는 임시 값 검증 뒤 교체하는 방식으로 원자적·멱등적이어야 한다. 손상되거나 미래 버전인 저장은 원문 백업을 보존한 채 안전한 새 게임으로 정규화한다.
...
- 제목: `H-17: NULL — 삭제된 밤`
- 장: `00:17 — 출석번호 없음` / `웃는 얼굴의 폭동` / `두 개의 학교` / `3초 승인실` / `증언 보관소` / `마지막 방송`
- 플레이 동사: `SIGNAL BLADE` / `DASH` / `REFLECT` / `TRACE`
- 결과 축: `Integrity` / `Exposure` / `Trust`

### 콘텐츠 안전 한계

- CS-01: 대상 연령은 초등학교 5~6학년이다.
