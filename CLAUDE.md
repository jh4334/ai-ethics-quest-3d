...
초등 5–6학년 AI 윤리 교육용 Three.js 오픈월드 게임. 2026-07부터 6장 독립 캠페인으로 전면 재구성 중.
...
- **에셋 출처 명시**: 직접 생성 이미지와 CC0/상업 사용 가능 에셋을 허용한다. 런타임에 쓰는 파일은
  `ASSET_LICENSES.md`에 출처·라이선스·사용 위치를 반드시 기록한다. 구매 에셋은 별도 승인 없이 추가하지 않는다.
...
- 오버월드 기준 톤은 남색 달빛 + 호박빛 기억(`toneMappingExposure=0.82`)이다. 던전은 자체 발광·조명으로
  가독성을 보정하고 안개는 방 너머(시작 24+)에서만 둔다.
- 모바일(터치) 레이아웃 유지: 이동 스틱(가상 조이스틱) 왼쪽, 확인/공격 A 버튼 오른쪽.
