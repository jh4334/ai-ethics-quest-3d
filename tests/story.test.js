...
test('침식 반격(N2): 안개 압력 2 이상이면 미해결 NPC 대화에 망각의 말이 앞선다', () => {
  ...
  assert.equal(getNpcDialog(solvedProgress, 'privacy').kind, 'closing');
});

test('가짜 도트(N3): 도구 2개=유인, 4개=만류, 기록되면 재발동 없음', async () => {
  const { FAKE_DOT_EVENTS, pendingFakeDotEvent, recordFakeDotEvent } = await import('../src/story.js');
  let progress = fresh();
  // 도구 0~1개: 조우 없음.
  assert.equal(pendingFakeDotEvent(progress), null);
  progress = { ...progress, tools: ['shield'] };
  assert.equal(pendingFakeDotEvent(progress), null);
  // 정확히 2개: 유인 조우.
  progress = { ...progress, tools: ['shield', 'mirror'] };
  assert.equal(pendingFakeDotEvent(progress), 'fake-dot-lure');
  // 'seen'만 기록돼도(선택 없이 닫음) 재발동하지 않는다.
  const seenOnly = recordFakeDotEvent(progress, 'fake-dot-lure', 'seen');
  assert.equal(pendingFakeDotEvent(seenOnly), null);
  // 선택 기록 후 4개가 되면 만류 조우.
  progress = recordFakeDotEvent(progress, 'fake-dot-lure', 'verify');
  assert.deepEqual(progress.fakeDotEvents, ['fake-dot-lure:verify']);
  progress = { ...progress, tools: ['shield', 'mirror', 'bell', 'compass'] };
  assert.equal(pendingFakeDotEvent(progress), 'fake-dot-plea');
  progress = recordFakeDotEvent(progress, 'fake-dot-plea', 'stop');
  assert.equal(pendingFakeDotEvent(progress), null);
  // 코어 완료 후에는 어떤 조우도 없다.
  const done = { ...fresh(), tools: ['shield', 'mirror', 'bell', 'compass'], aiCoreCompleted: true };
  assert.equal(pendingFakeDotEvent(done), null);
  // 이벤트 데이터: 각 조우는 속는 선택 1 + 판별 선택 1, 진짜 도트의 에필로그 보유.
  for (const event of Object.values(FAKE_DOT_EVENTS)) {
    assert.equal(event.options.filter((o) => o.fooled).length, 1);
    assert.equal(event.options.filter((o) => !o.fooled).length, 1);
    assert.ok(event.epilogueKo.length >= 1);
  }
  // 세이브 정규화: fakeDotEvents 중복 제거·타입 내성.
  const normalized = normalizeProgress({ fakeDotEvents: ['a', 'a', 7, null] });
  assert.deepEqual(normalized.fakeDotEvents, ['a']);
});
